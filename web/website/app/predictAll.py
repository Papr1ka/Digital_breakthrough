...
class automateTask:
    ...
    def predict(self, 
        ...
        if images:
            response['images'] = find_simmilar_pca(vectors)
        if group:
            response['group'] = predict_image_class(image, self.classifier)
        # Слишком много оперативки кушает, система убивает процесс, к сожалению без языковой модели
        #if description:
